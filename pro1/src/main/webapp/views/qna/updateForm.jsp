...
<%@include file="sidebar.jsp"  %>
	<div align="center" class="content">
		...
		<form method="post" action="uploadPro.jsp" onsubmit="return validationCheck()">
			<table width="400">
				<tr><td width="200">
					작성자&nbsp;&nbsp;&nbsp;세션의 writer
...
<% } %>				</select>&nbsp;&nbsp;&nbsp;&nbsp;
				...
				<tr><td colspan="2">제목 &nbsp;&nbsp;&nbsp;&nbsp;
					<input type="text" name="title" size="40" maxlength="100" value="<%=dto.getTitle()%>"required/></td>
				</tr>
			</table>
			<br>
			...
			<table width="800">
				<tr>
					...
					<td align="right">글 비밀번호를 입력해주세요&nbsp;&nbsp;
						<input type="password" name="ppw" maxlength="4" placeholder="숫자 4자" value="<%=dto.getPpw()%>" required/>
	...
	function validationCheck(){
	   	//summernote 부분에 입력된 데이터가 없을 때, 내용을 입력해달라는 validation
	   	let content = $('#summernote').summernote('code').trim();

	   	// summernote는 아무것도 입력하지 않으면 <p><br></p>와 같은 HTML 태그를 반환하는 경우가 많음
	   	// 이럴 땐 text 부분만 추출해서 비교하는 것이 안전함
	   	let plainText = $('<div>').html(content).text().trim();

	   	// 이미지 존재 여부 검사 (img 태그가 하나라도 있으면 true)
	  	let hasImage = /<img[\s\S]*?>/i.test(content);

	   	if (plainText === "" && !hasImage) {
	       alert("내용(텍스트 또는 사진)을 입력해 주세요.");
	       $('#summernote').summernote('focus');
	       return false;
	   	}
	// (필요시 추가 검사: 제목/상호명 등은 input 태그에서 required 속성이 있기 때문에 별도 검사 생략 가능)
   	// 모든 검사 통과 시 true 반환
  	 return true;
	}
</script>
